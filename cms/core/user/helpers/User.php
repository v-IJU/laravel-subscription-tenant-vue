...
class User
{
     ...
     * get active user group
     */
    static function getUserGroupName($id = false, $groupname = null)
    {
        if ($id == false) {
    ...
    static function hasRole($groupname = null)
    {
        $user = self::getUser();
        ...
        $group = [];
        foreach ($user->group as $groups) {
            $group[$groups["group"]] = $groups["group"];
        }

        return isset($group[$groupname]) ? true : false;
    }
    /*
     * get active user
     */
